...
//Clone
execSync(
  "git clone --depth 1 --filter=blob:none --no-checkout https://github.com/Ultimaker/Cura.git upstream"
);
...
for (const definition of rawExtruders) {
  ...
  renameSync(
    join("upstream/resources/extruders", definition),
    join("src/definitions", definition.toLowerCase())
  );
}
...
for (const definition of rawPrinters) {
  //Move to "src/definitions"
  renameSync(
    join("upstream/resources/definitions", definition),
    join("src/definitions", definition.toLowerCase())
  );
}
...
const imports = printers
  ...
  .map(
    (definition) =>
      `import * as ${definition.normalized} from './${definition.raw}';`
  )
  .join("\r\n");
...
console.log(
  `📝 Generated index for ${printers.length} printers and ${extruders.length} extruders!`
);
